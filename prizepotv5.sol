...
library Address {
    ...
    /**
     * @dev Sends `amount` wei to `recipient`, forwarding all available gas and reverting on errors.
     */
 ...
 */
    interface IUniswapV2Router02 {
        ...
        // Function to get output amount for given input amount
...
contract PrizePot is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public liquidityFeeBuy = 200; // 2%
    uint256 public marketingFeeBuy = 200; // 2%
    uint256 public teamFeeBuy = 200; // 2%
        
    // Fees for selling (in basis points)
    uint256 public liquidityFeeSell = 200; // 2%
    uint256 public marketingFeeSell = 200; // 2%
    uint256 public teamFeeSell = 400; // 4%

    // Distribution shares (in basis points)
    uint256 public liquidityShare = 400; // 4%
    uint256 public marketingShare = 400; // 4%
    uint256 public teamShare = 1600; // 16%

    // Total taxes (in basis points)
    uint256 public totalTaxIfBuying = liquidityFeeBuy + marketingFeeBuy + teamFeeBuy; // 6%
    uint256 public totalTaxIfSelling = liquidityFeeSell + marketingFeeSell + teamFeeSell; // 8%
    uint256 public totalDistributionShares = liquidityShare + marketingShare + teamShare; // 24%
    ...
    // Maximum fee limits
    uint256 public constant MAX_TOTAL_FEE = 2000; // Maximum total fee is 20%
    uint256 public constant MAX_INDIVIDUAL_FEE = 1000; // Maximum individual fee is 10%
    ...
    // Slippage tolerance (in percentage, e.g., 5 for 5%)
    uint256 public swapSlippage = 5; // 5%
    ...
    constructor () ReentrancyGuard() {
        ...
        marketingWallet = payable(0x7184eAC82c0C3F6bcdFD1c28A508dC4a18120b1e); // Marketing Address
        teamWallet = payable(0xa26809d31cf0cCd4d11C520F84CE9a6Fc4d4bb75); // Team Address
            
        // Initialize Uniswap router with the specified address
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
            0x10ED43C718714eb63d5aA57B78B54704E256024E // PancakeSwap Router on BSC
        ); 
    ...
    function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
        require(spender != address(0), "PrizePot: increase to zero");
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        require(spender != address(0), "PrizePot: decrease to zero");
        require(_allowances[_msgSender()][spender] >= subtractedValue, "PrizePot: decrease below zero");
    ...
    function approve(address spender, uint256 amount) public override returns (bool) {
        require(spender != address(0), "PrizePot: approve to zero");
    ...
    function _approve(address owner_, address spender, uint256 amount) private {
        require(owner_ != address(0), "PrizePot: approve from zero");
        require(spender != address(0), "PrizePot: approve to zero");

        // Optimize storage updates to prevent unnecessary writes
        if (_allowances[owner_][spender] != amount) {
    ...
    function setIsTxLimitExempt(address holder, bool exempt) external onlyOwner {
        if (isTxLimitExempt[holder] != exempt) {
    ...
    function setIsExcludedFromFee(address account, bool newValue) public onlyOwner {
        if (isExcludedFromFee[account] != newValue) {
    ...
    function setBuyTaxes(uint256 newLiquidityFeeBP, uint256 newMarketingFeeBP, uint256 newTeamFeeBP) external onlyOwner {
        require(newLiquidityFeeBP <= MAX_INDIVIDUAL_FEE, "PrizePot: Liquidity fee high");
        require(newMarketingFeeBP <= MAX_INDIVIDUAL_FEE, "PrizePot: Marketing fee high");
        require(newTeamFeeBP <= MAX_INDIVIDUAL_FEE, "PrizePot: Team fee high");

        uint256 totalFeeBP = newLiquidityFeeBP + newMarketingFeeBP + newTeamFeeBP;
        require(totalFeeBP <= MAX_TOTAL_FEE, "PrizePot: Total fee high");

        liquidityFeeBuy = newLiquidityFeeBP;
        marketingFeeBuy = newMarketingFeeBP;
        teamFeeBuy = newTeamFeeBP;

        totalTaxIfBuying = liquidityFeeBuy + marketingFeeBuy + teamFeeBuy;
    ...
    function setSellTaxes(uint256 newLiquidityFeeBP, uint256 newMarketingFeeBP, uint256 newTeamFeeBP) external onlyOwner {
        ...
        liquidityFeeSell = newLiquidityFeeBP;
        marketingFeeSell = newMarketingFeeBP;
        teamFeeSell = newTeamFeeBP;

        totalTaxIfSelling = liquidityFeeSell + marketingFeeSell + teamFeeSell;
    ...
    function setDistributionSettings(uint256 newLiquidityShareBP, uint256 newMarketingShareBP, uint256 newTeamShareBP) external onlyOwner {
        liquidityShare = newLiquidityShareBP;
        marketingShare = newMarketingShareBP;
        teamShare = newTeamShareBP;

        totalDistributionShares = liquidityShare + marketingShare + teamShare;
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner {
        require(maxTxAmount >= _totalSupply / 10000, "PrizePot: Max TX too low");
        require(maxTxAmount <= _totalSupply, "PrizePot: Max TX too high");
    ...
    function setIsWalletLimitExempt(address holder, bool exempt) external onlyOwner {
        if (isWalletLimitExempt[holder] != exempt) {
    ...
    function setWalletLimit(uint256 newLimit) external onlyOwner {
        require(newLimit >= _totalSupply / 10000, "PrizePot: Wallet limit too low");
        require(newLimit <= _totalSupply, "PrizePot: Wallet limit too high");
        if (_walletMax != newLimit) {
            _walletMax  = newLimit;
    ...
    function setNumTokensBeforeSwap(uint256 newLimit) external onlyOwner {
        if (minimumTokensBeforeSwap != newLimit) {
    ...
    /**
     * @dev Sets the slippage tolerance for Uniswap swaps.
     * @param newSlippage New slippage tolerance in percentage (e.g., 5 for 5%).
     */
    function setSwapSlippage(uint256 newSlippage) external onlyOwner {
        require(newSlippage <= 100, "PrizePot: Slippage too high");
        if (swapSlippage != newSlippage) {
            swapSlippage = newSlippage;
            emit SwapSlippageUpdated(newSlippage);
        }
    }
    ...
    function setSwapAndLiquifyByLimitOnly(bool newValue) external onlyOwner {
        if (swapAndLiquifyByLimitOnly != newValue) {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        require(_allowances[sender][_msgSender()] >= amount, "PrizePot: Allowance exceeded");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private nonReentrant returns (bool) {
        require(sender != address(0), "PrizePot: From zero");
        require(recipient != address(0), "PrizePot: To zero");
        require(amount > 0, "PrizePot: Zero amount");

        // Enforce max transaction limit
        if(!isTxLimitExempt[sender] && !isTxLimitExempt[recipient]) {
            require(amount <= _maxTxAmount, "PrizePot: Max TX");
        }            
        ...
        if(checkWalletLimit && !isWalletLimitExempt[recipient])
            require(balanceOf(recipient) + finalAmount <= _walletMax, "PrizePot: Max wallet");
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            feeAmount = (amount * totalTaxIfBuying) / 10000; // Buy fee
        }
        else if(isMarketPair[recipient]) {
            feeAmount = (amount * totalTaxIfSelling) / 10000; // Sell fee
        }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 tokensForLiquidity = (tAmount * liquidityShare) / totalDistributionShares / 2;
        ...
        uint256 totalBNBFee = totalDistributionShares - (liquidityShare / 2);
        
        uint256 amountBNBLiquidity = (transferredBalance * liquidityShare) / (totalBNBFee * 2);
        uint256 amountBNBMarketing = (transferredBalance * marketingShare) / totalBNBFee;
        uint256 amountBNBTeam = transferredBalance - amountBNBLiquidity - amountBNBMarketing;

        // Transfer ETH to marketing and team wallets
        ...
        emit SwapAndLiquify(tokensForSwap, transferredBalance, tokensForLiquidity);
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        uint256 amountOutMin = (expectedAmount * (100 - swapSlippage)) / 100; // Adjust based on slippage

        _approve(address(this), address(uniswapV2Router), tokenAmount);

        // Make the swap with adjusted slippage
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // Add the liquidity with minimal slippage protection
        uniswapV2Router.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
            (tokenAmount * (100 - swapSlippage)) / 100, // amountTokenMin based on slippage
            (ethAmount * (100 - swapSlippage)) / 100,   // amountETHMin based on slippage
            owner(),
            block.timestamp
        );
    }
     ...
     */
    // Function to allow owner to withdraw Ether
    function withdrawEther(uint256 amount) external onlyOwner nonReentrant {
        require(address(this).balance >= amount, "PrizePot: Insufficient ETH");
        transferToAddressETH(payable(owner()), amount);
    ...
    function withdrawERC20(address tokenAddress, uint256 tokenAmount) external onlyOwner nonReentrant {
        require(tokenAddress != address(this), "PrizePot: Can't withdraw own tokens");
    ...
    /**
     * @dev Emitted when swap slippage is updated.
     */
    event SwapSlippageUpdated(uint256 newSlippage);
}
