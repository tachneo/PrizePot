...
library SafeMath {
    ...
    // Other SafeMath functions remain unchanged
...
library Address {
    ...
    // Other Address functions remain unchanged
...
contract Ownable is Context {
    ...
    // Lock and unlock functions remain unchanged
    ...
    function unlock() public virtual {
        ...
        require(block.timestamp > _lockTime , "Contract is locked until specified time");
...
contract PRIZEPOT is Context, IERC20, Ownable {
    ...
    uint256 public _lotteryShare = 4;
    uint256 public lotteryPot;
    uint256 public lotteryThreshold = 100000000 * 10**_decimals; // Set your desired threshold
    ...
    constructor () {
        ...
        // Update total taxes and shares including lottery fees and shares
    ...
    function setLotteryThreshold(uint256 newThreshold) external onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if(checkWalletLimit && !isWalletLimitExempt[recipient])
                require(balanceOf(recipient).add(finalAmount) <= _walletMax, "Wallet limit reached");

            _balances[recipient] = _balances[recipient].add(finalAmount);

            // Add recipient to lottery participants if not already added
            if(!isParticipant[recipient] && !recipient.isContract()) {
                isParticipant[recipient] = true;
                lotteryParticipants.push(recipient);
            }

            // Check if lottery pot has reached the threshold
            if(lotteryPot >= lotteryThreshold) {
                pickLotteryWinner();
            }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountBNBLottery = amountReceived.sub(amountBNBLiquidity).sub(amountBNBTeam).sub(amountBNBMarketing);
        ...
        if(amountBNBLottery > 0)
            lotteryPot = lotteryPot.add(amountBNBLottery);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        // generate the uniswap pair path of token -> WETH (Wrapped ETH)
        path[0] = address(this);
        ...
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            path,
            address(this), // The contract receives ETH
            block.timestamp
        );
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        uniswapV2Router.addLiquidityETH{value: ethAmount}(
            ...
            0, // slippage is unavoidable
            owner(), // LP tokens are sent to owner
            block.timestamp
        );
    }
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        uint256 feeAmount = 0;
        uint256 lotteryFeeAmount = 0;
        
        if(isMarketPair[sender]) {
            feeAmount = amount.mul(_totalTaxIfBuying).div(100);
            lotteryFeeAmount = amount.mul(_buyLotteryFee).div(100);
        }
        else if(isMarketPair[recipient]) {
            feeAmount = amount.mul(_totalTaxIfSelling).div(100);
            lotteryFeeAmount = amount.mul(_sellLotteryFee).div(100);
        }
        
        if(feeAmount > 0) {
            uint256 contractFeeAmount = feeAmount.sub(lotteryFeeAmount);
            _balances[address(this)] = _balances[address(this)].add(contractFeeAmount);
            emit Transfer(sender, address(this), contractFeeAmount);

            lotteryPot = lotteryPot.add(lotteryFeeAmount);
            emit Transfer(sender, address(this), lotteryFeeAmount);
        }

        return amount.sub(feeAmount);
    }
    function pickLotteryWinner() internal {
        ...
        uint256 randomNumber = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, lotteryParticipants.length)));
        ...
        (bool success, ) = winner.call{value: lotteryPot}("");
        if(success) {
            emit LotteryWinner(winner, lotteryPot);
            lotteryPot = 0;
            // Reset participants
            for(uint256 i = 0; i < lotteryParticipants.length; i++) {
                isParticipant[lotteryParticipants[i]] = false;
            }
